...
func (i *ImageService) getBestDescriptorForExport(ctx context.Context, indexDesc ocispec.Descriptor) (ocispec.Descriptor, error) {
	...
	missingManifests := []ocispec.Descriptor{}
	presentManifests := []ocispec.Descriptor{}
	for _, mfst := range children {
		if containerdimages.IsManifestType(mfst.MediaType) {
			...
			if err != nil {
				missingManifests = append(missingManifests, mfst)
	...
	if len(missingManifests) == 0 {
		// If we have the full image, just export the original index.
